...
import 'client_order_listdetail_page.dart';
...
class _ClientOrdersListPageState extends State<ClientOrdersListPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    print('ClientOrdersListPage initState llamado');
  ...
  Future<void> fetchOrders({required String status}) async {
    ...
    print('ID de usuario Cliente: $userId');

    final response = await http.post(
      Uri.parse('https://www.aaservicek.com/servicek_backend/actions/orders_actions.php'),
      body: {
        'action': 'GET_CLIENT_ORDERS',
        'id_client': userId.toString(),
        'status': status,
      },
    );

    print('Datos enviados a GET_CLIENT_ORDERS:');
    print('id_client: ${userId.toString()}');
    print('status: $status');
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(
            'Cantidad de Ordenes (${orders.length})',
        ...
        return InkWell(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => ClientOrderListdetailPage(order: order),
              ),
            );
          },
          child: Card(
...
void main() {
  runApp(MaterialApp(
    title: 'Client Orders App',
    home: ClientOrdersListPage(),
  ));
}